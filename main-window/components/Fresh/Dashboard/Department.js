...
const department = ({
  ...
  const depObjs = _.uniq(
    _.flatten(
      _.flatten(lines.map(lID => schema.lines[lID].ranges)).map(
        rID => schema.ranges[rID].obj
      )
    )
  );
  ...
  const wireId = _.find(
    depObjs,
    id => depFresh[id].I === maxIce && depFresh[id].msg[0] === msgCode
  );
  ...
  return (
    <AccordionItem className={'dep_block'} uuid={`dep${depID}`}>
      <AccordionItemHeading>
        <AccordionItemButton>
          <AccordionItemState>
            {state => (
