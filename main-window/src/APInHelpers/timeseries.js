...
export const ts_UnderscoredString = (ts) => (moment.unix(ts).format('YYYY-MM-D_HH:mm:ss.SSS'))

export const tsmili__UnderscoredString = (ts) => (moment(ts).format('YYYY-MM-D_HH:mm:ss.SSS'))

//(_.max(...Object.keys(fresh).map(key => fresh[key].ts)))

// IN unix second OUT -//- minus x hrs
export const minusHrs = (uts, hours) => (moment.unix(uts).subtract(hours, 'hours').unix())
export const plusHrs = (uts, hours) => (moment.unix(uts).add(hours, 'hours').unix())
...
const formTrend = (trendMaterial, objId ,propName) => {
    const xyPairs = _.sortBy((trendMaterial
        .filter(tm => tm.obj === objId)
        .map(tm => [tm.ts, tm[propName]])), tm => tm[0]) // sort by ts
