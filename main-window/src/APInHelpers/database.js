...
export const thinDB = (spanSecs) => {
    return db.transaction("rw", db.data, async () => {
        ...
        while (spanStartTs >= _.min(abundantTss)) {
            ...
            const subset = _.filter(abundantTss, ts => ts <= spanEndTs && ts > spanStartTs);
...
export const writeDataToDB = (dataArr) => {
    ...
    return db.transaction("rw", db.data, async () => {
        ...
        const dbPKs = await db.data.where("[ts+obj]")         // primary keys existing in db
            .anyOf(incomingPKs)
        ...
        if (repeatedPKs.length > 0) {
            console.log(`%c[DB] Will not write ${repeatedPKs.length} slices`, 'color: purple');
        }
        ...
        if (missingPKs.length > 0) {
            return db.data.bulkAdd(dataToWrite)
                .then(() => {
                    console.log(`%c[DB] Have written ${missingPKs.length} slices`, 'color: purple');
                    return missingPKs
...
export const readDataByTSRanges = (objsArr, tsRange) => {
    ...
    return db.data.where("ts")
        .between(tsRange[0],tsRange[1], {includeLowers: true, includeUppers: true})
        .filter(d => (_.includes(objsArr, d.obj)))
        .toArray()
        .catch(err => {
            console.error(`Failed to to get data for objects ${objsArr.join(', ')}`, err);
        }) 
}
...
export const freshestPKs = (PKs) => {
    ...
    const res =  objIDs.map(o => (
        [
            _.max(_.filter(PKs, pk => pk[1] === o) // subset by object
                .map(pk => pk[0])),  // take obly tss and maximum
            o 
        ] 
    ))
...
const filterSlices = (PKs, dataArr) => (PKs.map(pk => {
        const ts = pk[0];
        const obj = pk[1];
