...
export const prepareHistoryRequest = async (needMin, needMax, objIDs, secsSpan) => {
    ...
    return readPKsByTSRanges(objIDs, needMin, needMax).then(
        pks => {
            ...
            return objIDs.map(o => {
                ...
                const segments = calcMissingSegments(gotTssArr, needMin, needMax, secsSpan);
...
export const subsetObj = (history, objID) => ( history.filter(h => (h.obj === parseInt(objID))) )
...
const calcExpectedTSs = (timespanRequest) => {
    ...
    if (tsStart < tsEnd) // проверка что правильная посоледовательность) 
    {
        let tss = [];
        let tsCur = tsEnd;
        while (tsCur >= tsStart) {
...
const getActualTSs = (subset) => {
    ...
    const duplicates = _.filter(tss, (value, index, iteratee) => _.includes(iteratee, value, index + 1))
    ...
    if (isAllUnique) {
        ...
        console.log(`[HISTORY ANALYZER] Warning = ${duplicates.join(', ')} were met several times in the subset`);
        return uniqueTss;
...
const firstLastN = (arr ,N) => ([_.take(arr, N).join(', '), _.takeRight(arr, N).join(', ')].join(' ... '))
