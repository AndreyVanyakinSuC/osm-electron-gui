...
export const geo_globalBounds = schema => {
  ...
  const res = [[_.min(lats), _.min(lons)], [_.max(lats), _.max(lons)]];
...
export const geo_towerRanges = (schema, towerID) => {
  ...
  const rangeIDs = _.filter(_.keys(schema.ranges), rID =>
    _.includes(schema.ranges[rID].towers, towerID)
  );

  return _.pick(schema.ranges, rangeIDs);
};

const findPeerTowerID = (towerIDsArr, towerID) =>
  _.find(towerIDsArr, twrID => twrID !== towerID);
