...
export const schemaAllObjectTypes = (schema) => _.sortBy(_.uniq(_.values(_.mapValues(schema.obj, o => o.Type))),['desc'])

export const schemaAllLines = (schema) => _.castArray(_.mapValues(schema.lines, l => l.Name))

//FIXME:
export const schemaFirstLineID = (schema) => (_.parseInt(_.keys(schema.lines)[0]))

// OUT rangeUD
export const schemaFirstRangeofLine = (schema, lineID) => (schema.lines[lineID].ranges[0])

export const schemaRangeObjects = (schema, rangeID) => (schema.ranges[rangeID].obj)

export const schemaRangeByObjectIds = (schema, objIDs) => _.uniq((objIDs.map(o => schema.obj[o].parentRange))).join('');
...
export const schemaTypesbyObjIDs = (schema, objIDs) => (objIDs.map(o => schema.obj[o].Type))
...
export const schemaLinebyRange = (schema, rangeID) => (schema.ranges[rangeID].parentLine)
