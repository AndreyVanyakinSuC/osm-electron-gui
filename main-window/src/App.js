...
import { maxTS, minusHrs, nowTS, addTrends, displayHuman } from './APInHelpers/timeseries';
...
class App extends Component {
  ...
  parseHistory( historyJson) {
    ...
    if (isEmpty) {
      console.log('%c[H] Server returned an empty arr => no history for the requested timespans');
      return;
    }
    ...
    const history = checkHistory(this.state.lastHistoryRequest, verifiedHistory);

    // 3) Write to db and clear the last history request
    writeDataToDB(history)
    .then(PKs => {
      if (PKs === null) {
  ...
  fillStateWithFresh(freshPKs, trendHours = TREND_HRS) {//
    ...
    readByPKs(freshPKs)
      ...
      .then(dataArr => {
        ...
        readDataByTSRanges(objIDs, trendRange)
          .then(trendMaterial => { // dataArr
  ...
  componentDidMount() {
    ...
    ipcRenderer.on('schema:new', (e, schemaJson) => {
      ...
      if (this.state.isSchemaAvailable) {
        ...
        this.setState(prevState => {
          ...
          const isSameSchema = compareSchemas(JSON.stringify(prevState.schema), schemaJson)

          if (isSameSchema) {
            console.log('Incoming schema is the same thats already in state');
            return null
          } else {
            console.log('Some schema was available but the received one is different, applyong it');
            writeSchemaToDB(schemaJson)
              .then(() => {
    ...
    ipcRenderer.on('history:request-fired', () => console.log('%c[IPC] history:request-fired', 'color: darkgreen'))
  ...
  async handleHistoryRequest(needMin, needMax, objIDs, spanSecs) {
    ...
    const request = await prepareHistoryRequest(needMin,needMax,objIDs,spanSecs);
    ...
    if (isEmptyRequest) {
      console.log('%c[h] The formed request was empty, i.e. no need to fetch data');
      return null;
    } 
    ...
    return (
      <div className="App">
        <Header 
          ...
          ribbonData = {isCanShowFresh ? worstCaseRibbon(this.state.fresh) : {value: null, msgCode: null}}
