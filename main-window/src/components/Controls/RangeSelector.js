...
const rangeSelector = ({ value, schema, selectedLine, changed }) => {
  ...
  const towersNameByRange = (id, schema) => [
    schema.towers[schema.ranges[id].towers[0]].number,
    schema.towers[schema.ranges[id].towers[1]].number
  ]; // ['2424','443']

  const renderRangeLabel = (id, schema) =>
    `№№ ${towersNameByRange(id, schema)[0]}-${
      towersNameByRange(id, schema)[1]
    }`; //
  ...
  const currentOption = {
    value: value,
    label: renderRangeLabel(value, schema)
  };
