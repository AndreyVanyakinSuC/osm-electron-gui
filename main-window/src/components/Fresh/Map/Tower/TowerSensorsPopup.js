...
import {
  geo_towerRanges,
  geo_pickRange,
  geo_adjacentTowerObject
} from '../../../../APInHelpers/map';
...
class TowerSensorsPopup extends Component {
  ...
  render() {
    ...
    const markup_statusPane = (rangeObject, position, fresh) => {
      ...
      const sortedObjs = _.sortBy(
        _.keys(rangeFresh),
        id => schema.obj[id].Type
      );
        ...
        <div className={classes}>
          {sortedObjs.map(id => {
            ...
            if (msgCode !== undefined) {
              ...
              display = (
                <span className="status" title="Опасность не обнаружена">
    ...
    if (adjRangesCount === 2) {
      ...
      const towardsTower = geo_adjacentTowerObject(
        towardsRange,
        towerID,
        schema
      );
    ...
    return (
      <Popup className="tower-popup">
        <div className="tower-popup-header">
          ...
          <div
            className="details-expand-toggler"
            onClick={this.handleToggleClick.bind(this)}
          >
            <ExpandToggle isExpanded={this.state.isDetailsExpanded} />
          </div>
        </div>
