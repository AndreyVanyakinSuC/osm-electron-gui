...
const rangeHeader = ({
  ...
  const towerNames = towerIDs.map(v => {
    return schema.towers[v].number;
  });
  const sensorsFlag = towerIDs.map(v => {
    return schema.towers[v].hasSensors;
  });

  // do not include undefind temp readings for missing fresh
  const thisRangeFreshObjIDs = _.filter(_.keys(fresh), k =>
    _.includes(schema.ranges[rangeID].obj, parseInt(k))
  );
  ...
  return (
    <div className={'range_header'}>
      ...
      />
      <RangeTemp temp={temp} title={`Температура воздуха на участке`} />
      <RangeChart
        rangeID={rangeID}
