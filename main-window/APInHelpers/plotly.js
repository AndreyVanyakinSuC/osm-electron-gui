import {
  ...
  unixMiliSecs,
  date_UTS,
  ts_UnderscoredString,
  tsmili__UnderscoredString
} from './timeseries';
...
export const yLayout = (prop, mode, yRange) => {
  ...
  let axisNo, color, title, rangemode, zeroline, fixedrange, side, overlaying;
...
export const averageXYpairs = xyPairsArr => {
  ...
  // console.log(flat);
  const getMean = ts =>
    _.mean(flat.filter(xy => xy[0] === ts).map(xy => xy[1])); // Tamb
  // [ts, ts, ts, ts]
...
const XYpairstoXYarrs = xyPairs => [
  _.flatten(xyPairs.map(pair => [pair[0]])),
  _.flatten(xyPairs.map(pair => [pair[1]]))
];
...
export const displayName_wireID = (displayName, objData) => {
  const [type, name] = [...ENTITY_NAMES.entries()].find(entry => {
