...
log.transports.console.format = '[{y}-{d}-{m} {h}:{i}:{s}.{ms}] [{label}] [{level}] {text}'
log.transports.file.format = '[{y}-{d}-{m} {h}:{i}:{s}.{ms}] [{label}] [{level}] {text}'
    ...
    ipcMain.on(ELECTRON_HISTORYREQ, () => {
        ...
        return reqHistory()
            .then(res => {
                ...
                mainWindow.webContents.send(MAINWINDOW__HISTORYRES,  JSON.stringify(res.data))
            })
            .catch(err => {
